    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            if len(replay_memory) == replay_memory_size:
                ...
                '''
                def pop_memory(replay_memory):
                    '''
                    Input
                        replay_memory: list, element type:
                            namedtuple("Transition", ["state", "action", "reward", "next_state", "done"])
                    Outpu
                        replay_memory: remove the memory of lowest reward
                    '''
                    min_reward = np.inf
                    for t in replay_memory:
                        r = t.reward
                        if r < min_reward:
                            min_reward = r
                    ...
                    flag = True
                    for t in replay_memory:
                        if t.reward == min_reward and flag:
                            flag = False
                            continue
                        new_memory.append(t)
